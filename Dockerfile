...
RUN apt-get update \
  ...
  && wget https://github.com/LiveHelperChat/livehelperchat/archive/refs/tags/3.92v.zip -O /tmp/source.zip \
  && unzip /tmp/source.zip -d /tmp/source/ \
  && mv /tmp/source/livehelperchat-3.92v/lhc_web/* /var/www/html/ \
