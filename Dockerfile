...
RUN apt-get update \
  ...
  && wget https://github.com/LiveHelperChat/livehelperchat/archive/refs/tags/3.72v.zip -O /tmp/source.zip \
  && unzip /tmp/source.zip -d /tmp/source/ \
  && mv /tmp/source/livehelperchat-3.72v/lhc_web/* /var/www/html/ \
